""""
NASA test case atmos 03: Dragless tumbling brick with aerodynamic damping (but no aero force).
"""
...
def test_case03(plotting=False):
    FT_TO_M = 0.3048
    SLUGFT2_TO_KGM2 = 1.35581795
    SLUG_TO_KG = 14.5939

    # Brick parameters (dragless)
    Cd = 0
    S = 0.22222 * FT_TO_M**2  # m^2
    b = 0.33333 * FT_TO_M  # m
    c = 0.66667 * FT_TO_M  # m
    m = 0.155404754 * SLUG_TO_KG  # kg
    Ixx = 0.001894220*SLUGFT2_TO_KGM2   # kg m^2
    Iyy = 0.006211019*SLUGFT2_TO_KGM2   # kg m^2
    Izz = 0.007194665*SLUGFT2_TO_KGM2   # kg m^2
    ...
    for i in range(nasa_sims_N):
        ...
        data_path_csv = "Atmos_03_TumblingBrickDamping/Atmos_03_sim_0{}.csv".format(i+1)
        ...
        nasa_local_g = data["localGravity_ft_s2"] * FT_TO_M
    ...
    sim.phases[0].modules["AeroConstant"].bref = b
    sim.phases[0].modules["AeroConstant"].cref = c
    sim.phases[0].modules["AeroConstant"].sref = S
    sim.phases[0].modules["AeroConstant"].c_ll_p = C_l_p
    sim.phases[0].modules["AeroConstant"].c_m_q = C_m_q
    sim.phases[0].modules["AeroConstant"].c_ln_r = C_n_r
    ...
    nasa_f2 = np.array([np.array(nasa_data[6]["nasa_phi"])[-1], np.array(nasa_data[6]["nasa_theta"])[-1],
                    np.array(nasa_data[6]["nasa_psi"])[-1]])
    errs2 = np.sqrt((sim_f2[0] - nasa_f2[0])**2), np.sqrt((sim_f2[1] - nasa_f2[1])**2), np.sqrt((sim_f2[2] - nasa_f2[2])**2)

    assert np.amax(errs) < 1 
    assert np.amax(errs2) < 1
